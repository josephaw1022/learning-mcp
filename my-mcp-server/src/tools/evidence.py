...
BASE_URL = os.getenv("QUARKUS_BASE_URL", "http://localhost:8080").rstrip("/") + "/evidence"
...
async def search_evidence(case_id: Optional[str] = None, status: Optional[str] = None) -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        for item in items[:10]: # Limit to top 10 for brevity
            output.append(f"- UUID: {item['uuid']}, Filename: {item['filename']}, Status: {item['status']}, Case: {item['caseId']}")
...
async def get_evidence_history(evidence_uuid: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        for entry in entries:
            output.append(f"[{entry['timestamp']}] Actor: {entry['actorId']}, Action: {entry['actionType']}, Notes: {entry['notes']}")
...
async def transfer_evidence(evidence_uuid: str, from_user: str, to_user: str, reason: str, actor_id: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        response = await client.patch(f"{BASE_URL}/{evidence_uuid}/transfer", json=payload)
        if response.status_code == 404:
            return f"Evidence item {evidence_uuid} not found."
        response.raise_for_status()
...
async def verify_evidence(evidence_uuid: str, actor_id: str) -> str:
    ...
    """
    async with httpx.AsyncClient() as client:
        response = await client.post(f"{BASE_URL}/{evidence_uuid}/verify", params={"actorId": actor_id})
        if response.status_code == 404:
            return f"Evidence item {evidence_uuid} not found."
