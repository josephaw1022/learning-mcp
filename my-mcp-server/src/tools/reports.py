...
BASE_URL = get_env_var("QUARKUS_BASE_URL", "http://localhost:8080").rstrip("/") + "/reports"
...
async def find_compromised_evidence() -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        for item in items:
            output.append(
                f"- UUID: {item['uuid']}, File: {item['filename']}, Case: {item['caseId']}, Current Status: {item['status']}"
            )
...
async def get_officer_workload_analysis() -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        for s in stats:
            output.append(
                f"- {s['fullName']} (Badge: {s['badgeNumber']}): {s['itemCount']} active items"
            )
