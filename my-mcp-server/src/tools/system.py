...
BASE_URL = os.getenv("QUARKUS_BASE_URL", "http://localhost:8080").rstrip("/") + "/system"
...
async def list_storage_facilities() -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        for f in facilities:
            ...
            output.append(f"- {f['name']} (ID: {f['facilityId']}) - {status} - {f['address']}")
        return "\n".join(output)
...
async def list_storage_locations(facility_id: Optional[str] = None) -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        for loc in locations[:20]:
            output.append(f"- {loc['label']} (ID: {loc['id']}) Type: {loc['type']}, Capacity: {loc['capacity']}")
...
async def get_system_alerts(unresolved_only: bool = True) -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        for a in alerts[:15]:
            output.append(f"[{a['alertTime']}] {a['severity']}: {a['message']} (Resolved: {a['isResolved']})")
        return "\n".join(output)
...
async def list_officers(department: Optional[str] = None) -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        for o in officers[:20]:
            output.append(f"- {o['fullName']} (Badge: {o['badgeNumber']}) Role: {o['role']}, Dept: {o['departmentName']}")
